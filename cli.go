...
type CLI struct {
	...
	NoBot           bool     "help:\"Exclude bot authors (may return fewer than `--limit`)\"      short:\"B\" aliases:\"no-bots\"                                                                          clib:\"terse='Exclude bots',group='Filters/3'\""
	...
	Interval    *time.Duration "help:\"Override auto-refresh interval for `--interactive` or `--watch`\" clib:\"terse='Refresh interval',group='Interactive/0'\" placeholder:\"<duration>\""

	Approve      bool   `help:"Approve each PR"                                                 clib:"terse='Approve PRs',group='Interactive/1'"`
	Close        bool   `help:"Close each PR"                                                   clib:"terse='Close PRs',group='Interactive/1'"`
	Copilot      bool   `help:"Request Copilot review on each PR"                               clib:"terse='Copilot review',group='Interactive/1'"`
	DeleteBranch bool   "help:\"Delete branch after close (requires `--close`)\"                                                            clib:\"terse='Delete branch',group='Interactive/1'\""
	Comment      string `help:"Add a comment to each PR"                                        clib:"terse='Add comment',group='Interactive/1'"       placeholder:"<body>"`
	Edit         bool   `help:"Edit title and body of each PR"                                  clib:"terse='Edit PR',group='Interactive/1'"                                short:"e"`
	MarkDraft    bool   `help:"Convert each PR to draft (only targets non-draft PRs)"           clib:"terse='Convert to draft',group='Interactive/1'"`
	MarkReady    bool   `help:"Mark each PR as ready for review (only targets draft PRs)"       clib:"terse='Mark as ready',group='Interactive/1'"`
	Merge        *bool  `help:"Toggle auto-merge (squash) on each PR"                           clib:"terse='Auto-merge',group='Interactive/1'"                                       negatable:""`
	ForceMerge   bool   `help:"Poll for checks, then force-merge (requires bypass permissions)" clib:"terse='Force-merge',group='Interactive/1'"                            short:"M"`
	Unsubscribe  bool   `help:"Remove review request and unsubscribe from each PR"              clib:"terse='Unsubscribe',group='Interactive/1'"                            short:"U"`
	Update       bool   `help:"Update each PR branch from base branch"                          clib:"terse='Update branch',group='Interactive/1'"`
	...
	// Output flags
	Watch    bool    `help:"Refresh output periodically"                                                                                  short:"W" clib:"terse='Watch mode',group='Output/0'"`
	ExitZero bool    `help:"Exit immediately when there are no results"                                                                   short:"0" clib:"terse='Exit on no match',group='Output/0'"                                                                                                                   name:"exit-0"`
	Columns  CSVFlag `help:"Table columns [index, ref, repo, owner, number, title, labels, author, state, review, created, updated, url]"           clib:"terse='Table columns',complete='predictor=columns,comma',group='Output/1'"                                                                                                 aliases:"col" placeholder:"<cols>"`
	Limit    *int    `help:"Maximum results"                                                                                              short:"L" clib:"terse='Max results',group='Output/1'"                                                                                                                                                    placeholder:"<n>"`
	Output   *string `help:"Output format"                                                                                                short:"o" clib:"terse='Output format',complete='values=url bullet table json repo',group='Output/1',enum='url,bullet,table,json,repo',highlight='u,b,t,j,r',default='table'"                             placeholder:"<fmt>"`
	Reverse  bool    `help:"Show oldest first (top)"                                                                                                clib:"terse='Reverse display order',group='Output/1'"`
	Sort     *string `help:"Sort by"                                                                                                                clib:"terse='Sort field',complete='values=name created updated',group='Output/1',enum='name,created,updated',highlight='n,c,u',default='name'"                                                 placeholder:"<field>"`

	// Miscellaneous
	Init    bool           `help:"Initialize config with defaults"            clib:"terse='Initialize config',group='Miscellaneous/0'"`
	Color   clog.ColorMode `help:"When to use color"                          clib:"terse='Color mode',group='Miscellaneous/1'"        default:"auto" enum:"auto,always,never"`
	Debug   bool           `help:"Log HTTP requests to stderr"                clib:"terse='Debug mode',group='Miscellaneous/1'"`
	Quick   bool           `help:"Skip enrichment (merge status, auto-merge)" clib:"terse='Skip enrichment',group='Miscellaneous/1'"                                           short:"Q"`
...
func (c *CLI) Validate() error {
	...
	if c.Interval != nil && !c.Interactive && !c.Watch {
		return fmt.Errorf("--interval requires --interactive or --watch")
	}
